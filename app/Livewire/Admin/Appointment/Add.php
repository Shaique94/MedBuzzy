...
class Add extends Component
{
    ...
    protected $rules = [
        ...
        'newPatient.email' => 'required|email|max:255',
        'newPatient.phone' => 'required|string|digits:10',
        'newPatient.gender' => 'required|string|in:male,female,other',
        'newPatient.age' => 'nullable|integer|min:1|max:120',
        'newPatient.pincode' => 'nullable|digits:6',
        'newPatient.address' => 'nullable|string|max:500',
        'newPatient.city' => 'nullable|string|max:255',
        'newPatient.state' => 'nullable|string|max:255',
        'newPatient.country' => 'nullable|string|max:255',
    ...
    public function updatedNewPatientPhone($value)
    {
        ...
        if (strlen($cleanValue) === 10) {
            $existingPatient = User::where('phone', $cleanValue)->first();
            if ($existingPatient) {
    ...
    /**
     * Normalize patient data by converting empty strings to null for nullable fields
     */
    private function normalizePatientData()
    {
        $normalizedData = [];
        foreach ($this->newPatient as $key => $value) {
            if (in_array($key, ['age', 'pincode', 'address', 'district', 'state', 'country'])) {
                // Convert empty strings to null for nullable fields
                $normalizedData[$key] = !empty($value) ? $value : null;
                
                // Special handling for age - ensure it's an integer or null
                if ($key === 'age' && $normalizedData[$key] !== null) {
                    $normalizedData[$key] = (int)$normalizedData[$key];
                }
            } else {
                // Keep required fields as-is (name, email, phone, gender)
    ...
    {
        $this->validate([
            'doctor_id' => $this->rules['doctor_id'],
            'appointment_date' => $this->rules['appointment_date'],
        ...
        // For cash payments, create appointment directly
        try {
        ...
        $normalizedPatient = $this->normalizePatientData();
        $email = $normalizedPatient['email'];
        $user = User::firstOrCreate(
            ['phone' => $normalizedPatient['phone']],
            [
                'name' => $normalizedPatient['name'] ?? 'Patient',
                'email' => $email,
        ...
        $patient = Patient::updateOrCreate(
            [
                'user_id' => $user->id,
                'name' => $normalizedPatient['name'] ?? $user->name,
            ],
            [
                'email' => $normalizedPatient['email'] ?? $user->email,
                'gender' => $normalizedPatient['gender'] ?? $user->gender,
                'age' => $normalizedPatient['age'],
                'pincode' => $normalizedPatient['pincode'],
                'address' => $normalizedPatient['address'],
                'district' => $normalizedPatient['district'],
                'state' => $normalizedPatient['state'],
                'country' => 'India',
            ]
        );
     ...
     public function createOrder()
{
    ...
    try {
        ...
        // Ensure User exists
        $email = $normalizedPatient['email'] ?? 'guest+' . ($normalizedPatient['phone'] ?? time()) . '@medbuzzy.local';
        $user = User::firstOrCreate(
            ['phone' => $normalizedPatient['phone']],
            [
                'name' => $normalizedPatient['name'] ?? 'Patient',
    ...
    public function handlePaymentSuccess($paymentId = null, $eventData = null)
    {
        ...
        if (!$paymentId) {
         ...
         try {
            ...
            // Dispatch event to refresh appointment list
            $this->dispatch('appointmentFailed');
