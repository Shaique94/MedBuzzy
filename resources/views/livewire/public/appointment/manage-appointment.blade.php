<div class="p-4 sm:p-6 max-w-5xl mx-auto bg-white rounded-2xl shadow-xl">
    <!-- Step Progress - Improved for mobile -->
        ...
        @foreach ([1 => 'Doctor', 2 => 'Date & Time', 3 => 'Patient', 4 => 'Confirm'] as $i => $label)
            <div class="flex-1 flex flex-col items-center relative">
                <div
                    ...
                    font-bold text-sm sm:text-base mb-1 sm:mb-2 transition-all duration-300 relative z-10">
                    {{ $i }}
                    @if ($step === $i)
                        <div
                ...
                <span
                    class="text-xs font-medium {{ $step >= $i ? 'text-brand-teal-600' : 'text-gray-400' }} hidden sm:block">
                    {{ $label }}
                </span>
                <span
                    class="text-[10px] font-medium {{ $step >= $i ? 'text-brand-teal-600' : 'text-gray-400' }} sm:hidden">
                    {{ substr($label, 0, 3) }}
                </span>
            </div>
            @if ($i < 4)
                <div
                    class="flex-1 h-1 mx-1 sm:mx-2 {{ $step > $i ? 'bg-brand-teal-600' : 'bg-brand-orange-100' }} transition-colors duration-300 relative top-3 sm:top-5">
                </div>
            @endif
        @endforeach
    </div>
    ...
    @if ($step === 1)
        <div class="space-y-4 sm:space-y-6">
            <!-- Department Filter - Improved mobile layout -->
            <div class="bg-brand-teal-50 p-3 sm:p-4 md:p-6 rounded-xl border border-brand-teal-100">
                <h2 class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center">
                    <svg class="h-5 w-5 sm:h-6 sm:w-6 mr-2 text-brand-teal-600" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" wire:click="$set('selectedDepartment', null)"
                        class="px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-colors duration-200 shadow-sm
                    ...
                    @foreach ($departments as $department)
                        <button type="button" wire:click="$set('selectedDepartment', {{ $department->id }})"
                            ...
                            {{ strlen($department->name) > 12 ? substr($department->name, 0, 10) . '...' : $department->name }}
                        </button>
                    @endforeach
                </div>
            </div>

            <!-- Doctors Grid - Improved mobile cards -->
            <div class="bg-white p-3 sm:p-4 md:p-6 rounded-xl border border-gray-100">
                <h2
                    class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-3 sm:mb-4 flex items-center justify-between">
                    <span class="flex items-center">
                        ...
                        {{ $doctor_id ? 'Selected' : 'Choose' }}
                    </span>
                    @if ($doctor_id)
                        <button wire:click="$set('doctor_id', null)"
                            class="text-xs sm:text-sm text-brand-teal-600 hover:text-brand-teal-800 flex items-center">
                            <svg class="h-3 w-3 sm:h-4 sm:w-4 mr-1" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                        @foreach ($doctors as $doctor)
                            <div wire:click="$set('doctor_id', {{ $doctor->id }})"
                                class="relative flex flex-col h-full border rounded-lg sm:rounded-xl overflow-hidden cursor-pointer transition-all duration-300 shadow-sm hover:shadow-md
                                ...
                                <div
                                    class="bg-gradient-to-br from-brand-teal-50 to-brand-orange-50 p-3 sm:p-4 md:p-5 text-center">
                                    <div class="relative">
                                        <div
                                            class="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 bg-brand-teal-600 text-white text-[10px] sm:text-xs px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full shadow-md">
                                            ₹{{ $doctor->fee }}
                                        </div>
                                        <div
    ...
    @endif
                                    ...
                                    <h3
                                        class="text-gray-900 font-bold mt-2 sm:mt-3 md:mt-4 text-sm sm:text-base md:text-lg">
                                        Dr. {{ $doctor->user->name }}</h3>
                                    <p class="text-brand-teal-600 text-xs sm:text-sm font-medium mb-1 sm:mb-2">
                                        {{ $doctor->department->name ?? 'General' }}</p>
                                    <div class="mt-2 sm:mt-3 flex flex-wrap justify-center gap-1">
                                        @foreach (['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] as $index => $day)
                                            ...
                                            @endphp
                                            <div class="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full text-[10px] sm:text-xs font-medium
                                    ...
                                    <button
                                        class="mt-2 sm:mt-3 w-full py-1.5 sm:py-2 bg-brand-teal-600 hover:bg-brand-teal-700 text-white rounded-lg text-xs sm:text-sm font-medium transition-colors shadow-sm">
                                        Select
                                    </button>
                                </div>
                            </div>
                ...
                @else
                    <div
                        class="flex flex-col items-center justify-center py-8 sm:py-10 md:py-12 text-gray-500 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                        <svg class="h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 mb-3 sm:mb-4 text-gray-400" fill="none"
                        ...
                        <p class="text-sm sm:text-base text-gray-600 mb-1 sm:mb-2">No doctors available</p>
                        <button wire:click="$set('selectedDepartment', null)"
                            class="text-xs sm:text-sm text-brand-teal-600 font-medium hover:text-brand-teal-700 flex items-center">
                            <svg class="h-3 w-3 sm:h-4 sm:w-4 mr-1" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
    ...
    @if ($step === 2)
        <div class="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
            @if ($selectedDoctor)
                <div
                    class="bg-gradient-to-r from-brand-teal-50 to-brand-teal-100 p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl shadow-lg border border-brand-teal-200">
                    <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 md:gap-6">
                        <div
                            class="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full overflow-hidden bg-white border-2 sm:border-4 border-brand-teal-600 shadow-lg">
                ...
                <div class="bg-white p-3 sm:p-4 md:p-6 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm">
                    <h2 class="text-lg sm:text-xl font-semibold text-gray-800 mb-4 sm:mb-5 md:mb-6 flex items-center">
                        <svg class="h-5 w-5 sm:h-6 sm:w-6 mr-2 text-brand-teal-600" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($hasAvailableDays)
                        ...
                        @if (!$appointment_date)
                            <div class="flex items-center justify-between mb-4 sm:mb-5 md:mb-6">
                                <button wire:click="previousMonth"
                                    class="px-3 py-1.5 sm:px-4 sm:py-2 bg-brand-teal-100 text-brand-teal-800 rounded-lg hover:bg-brand-teal-200 transition flex items-center text-xs sm:text-sm">
                                    <svg class="h-3 w-3 sm:h-4 sm:w-4 mr-1" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 19l-7-7 7-7" />
                                ...
                                <h3 class="text-sm sm:text-base md:text-lg font-medium text-gray-800">
                                    {{ \Carbon\Carbon::parse($currentMonth)->format('F Y') }}</h3>
                                <button wire:click="nextMonth"
                                    class="px-3 py-1.5 sm:px-4 sm:py-2 bg-brand-teal-100 text-brand-teal-800 rounded-lg hover:bg-brand-teal-200 transition flex items-center text-xs sm:text-sm">
                                    Next
                                    <svg class="h-3 w-3 sm:h-4 sm:w-4 ml-1" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 5l7 7-7 7" />
                                    </svg>
                            ...
                            <div
                                ...
                                <div
                                    class="grid grid-cols-7 gap-1 text-center text-xs sm:text-sm font-semibold text-gray-600 bg-gray-50 py-2 sm:py-3">
                                    @foreach (['S', 'M', 'T', 'W', 'T', 'F', 'S'] as $day)
                                        <div class="px-1 sm:px-2">{{ $day }}</div>
                                    @endforeach
                                </div>
                                <div class="grid grid-cols-7 gap-1 text-center text-xs sm:text-sm p-1 sm:p-2">
                                    @for ($i = 0; $i < $startDayOfWeek; $i++)
                                        <div class="bg-gray-50 rounded-lg h-8 sm:h-10 md:h-12"></div>
                                    @endfor
                                    @for ($day = 1; $day <= $daysInMonth; $day++)
                                        @php
                                            $dateObj = \Carbon\Carbon::parse($currentMonth)
                                                ->startOfMonth()
                                                ->addDays($day - 1);
                                        ...
                                        <button wire:key="day-{{ $day }}"
                                            ...
                                            class="w-full h-8 sm:h-10 md:h-12 rounded-lg transition-all duration-200 flex flex-col items-center justify-center
                                            {{ $isSelected ? 'bg-brand-teal-600 text-white font-bold ring-1 sm:ring-2 ring-brand-teal-400' : '' }}
                                            ...
                                            {{ $isToday ? 'border-1 sm:border-2 border-blue-400 text-blue-600 font-semibold' : '' }}"
                                            title="{{ $isOnLeave ? 'Doctor on leave' : (!$isAvailableDay ? 'Not available' : ($isBookable ? 'Available' : ($isPast ? 'Past date' : 'Outside booking range'))) }}">
                                            {{ $day }}
                                            @if ($isToday)
                                                <span class="text-[8px] sm:text-[10px] text-blue-600">Today</span>
                                            @endif
                                        </button>
                                    @endfor
                                </div>
                            ...
                            <div
                                class="mt-4 sm:mt-5 md:mt-6 flex flex-wrap gap-2 sm:gap-3 md:gap-4 justify-center text-[10px] sm:text-xs text-gray-600">
                                <div class="flex items-center">
                                    <div
                                        class="w-2 h-2 sm:w-3 sm:h-3 bg-brand-teal-50 border border-brand-teal-200 rounded-full mr-1 sm:mr-2">
                                    </div>
                                    <span>Available</span>
                                </div>
                                <div class="flex items-center">
                                    <div
                                        class="w-2 h-2 sm:w-3 sm:h-3 bg-brand-orange-100 border border-brand-orange-300 rounded-full mr-1 sm:mr-2">
                                    </div>
                                    <span>On Leave</span>
                                </div>
                                <div class="flex items-center">
                                    <div
                                        class="w-2 h-2 sm:w-3 sm:h-3 bg-red-100 border border-red-300 rounded-full mr-1 sm:mr-2">
                                    </div>
                                    <span>Not Available</span>
                                </div>
                                <div class="flex items-center">
                                    <div
                                        class="w-2 h-2 sm:w-3 sm:h-3 bg-gray-100 border border-gray-200 rounded-full mr-1 sm:mr-2">
                                    </div>
                                    <span>Past</span>
                                </div>
                                <div class="flex items-center">
                                    <div
                                        class="w-2 h-2 sm:w-3 sm:h-3 border-1 sm:border-2 border-blue-400 rounded-full mr-1 sm:mr-2">
                                    </div>
                                    <span>Today</span>
                                </div>
                            </div>
                        ...
                        @if ($appointment_date)
                            <div
                                class="mt-4 sm:mt-5 md:mt-6 bg-white p-3 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl border border-gray-100 shadow-lg">
                                <div
                                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4 gap-3 sm:gap-4">
                                    <h2
                                        class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 flex items-center">
                                        <svg class="h-5 w-5 sm:h-6 sm:w-6 mr-2 text-brand-orange-500" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div wire:loading wire:target="setAppointmentDate"
                                    class="py-6 sm:py-8 text-center text-gray-500">
                                    <div
                                        class="animate-spin rounded-full h-8 w-8 sm:h-10 sm:w-10 border-t-2 border-brand-teal-500 border-r-2 border-brand-teal-500 border-b-2 border-transparent mx-auto mb-3 sm:mb-4">
                                    </div>
                                    <p class="text-sm sm:text-base">Loading time slots...</p>
                                </div>
                                <div wire:loading.remove wire:target="setAppointmentDate"
                                    ...
                                    <div>
                                        <h3
                                            class="text-xs sm:text-sm font-semibold text-gray-600 mb-2 sm:mb-3 md:mb-4 flex items-center">
                                            <svg class="h-4 w-4 sm:h-5 sm:w-5 mr-2 text-brand-teal-600" fill="none"
                                                viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
                                            ...
                                            @foreach ($this->availableSlots as $time => $slot)
                                                ...
                                                @if ($isMorning)
                                                    <button wire:click="selectTimeSlot('{{ $time }}')"
                                                        ...
                                                        class="p-2 sm:p-3 border rounded-lg text-center transition-all duration-200 shadow-sm
                                                        ...
                                                        <div class="text-[10px] sm:text-xs mt-1">
                                            ...
                                            @if (!$hasMorning)
                                                <div
                                                    class="col-span-full py-4 sm:py-6 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                                    <p class="text-xs sm:text-sm text-gray-500">No morning slots</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
                                            ...
                                            @if (!$hasAfternoon)
                                                <div
                                                    class="col-span-full py-4 sm:py-6 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                                    <p class="text-xs sm:text-sm text-gray-500">No afternoon slots</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
                                            ...
                                            @if (!$hasEvening)
                                                <div
                                                    class="col-span-full py-4 sm:py-6 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                                    <p class="text-xs sm:text-sm text-gray-500">No evening slots</p>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                                    @if (empty($this->availableSlots))
                                        <div
                                            class="py-6 sm:py-8 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                            <p class="text-xs sm:text-sm text-gray-500">No time slots available</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    @else
                        <div class="text-center py-10 bg-red-50 rounded-2xl border border-red-200">
                            <svg class="h-14 w-14 mx-auto text-red-500 mb-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                            </svg>
                            <h3 class="text-xl font-medium text-red-700">No Availability Set</h3>
                            <p class="text-sm text-red-600">Please contact the doctor's office for scheduling options.
                            </p>
                        </div>
                    @endif
                </div>
            @else
                <div class="text-center py-10 bg-blue-50 rounded-2xl border border-blue-200">
                    <svg class="h-14 w-14 mx-auto text-blue-500 mb-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
    ...
    @if ($step === 3)
        <div class="space-y-8 max-w-4xl mx-auto">
            <div
                class="bg-gradient-to-r from-brand-teal-50 to-brand-teal-100 p-6 rounded-2xl shadow-lg border border-brand-teal-200">
                <div class="flex flex-col sm:flex-row items-center gap-6">
                    <div
                        class="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden bg-white border-4 border-brand-teal-600 shadow-lg">
            ...
            <div class="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-sm">
                <h2 class="text-xl font-semibold text-gray-800 mb-6 flex items-center">
                    <svg class="h-6 w-6 mr-2 text-brand-teal-600" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
    ...
    @if ($step === 4)
        <div class="space-y-6 max-w-4xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-12 h-12 rounded-full overflow-hidden bg-white border-2 border-brand-teal-200">
                                ...
                                <div>
                                    <p class="text-base font-semibold text-gray-800">Dr.
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-xs text-gray-600">{{ $newPatient['age'] }} yrs,
                                {{ ucfirst($newPatient['gender']) }}</p>
                        </div>
                        <div class="space-y-2">
                            <p class="text-sm font-medium text-gray-500">Contact</p>
                            <p class="text-sm text-gray-800">{{ $newPatient['email'] ?: 'Not provided' }}</p>
            ...
            <button wire:click="createOrder" wire:loading.attr="disabled"
                class="w-full bg-brand-teal-600 text-white px-6 py-3 rounded-lg hover:bg-brand-teal-700 disabled:opacity-50 flex items-center justify-center shadow-md">
                <span wire:loading.remove>Pay ₹50.00 & Confirm Appointment</span>
                <span wire:loading>
