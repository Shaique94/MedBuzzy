<div class="min-h-screen bg-gradient-to-br from-teal-50 via-blue-50 to-indigo-50 py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg border border-teal-100">
            ...
            <!-- Appointment card -->
            <div class="bg-gradient-to-br from-teal-50 to-blue-50 p-4 sm:p-6 rounded-xl mb-6 sm:mb-8 border border-teal-50 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
                    <h3 class="font-bold text-lg sm:text-xl text-teal-900">Appointment Summary</h3>
                    <span class="px-3 py-1 bg-teal-100 text-teal-800 text-sm rounded-full font-medium w-fit">Confirmed</span>
                </div>
                
                <div class="space-y-4 sm:space-y-6">
                    <div class="flex items-start space-x-3 sm:space-x-4">
                        <div class="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-teal-100 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-teal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-xs sm:text-sm font-medium text-gray-500">Doctor</p>
                            <p class="text-base sm:text-lg font-semibold text-gray-900 truncate">{{ $appointment->doctor->user->name ?? '' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start space-x-3 sm:space-x-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-base sm:text-lg font-semibold text-gray-900">
                                ...
                                <p class="text-sm sm:text-base text-teal-600">{{ \Carbon\Carbon::parse($appointment->appointment_time)->format('g:i A') }}</p>
                            </div>
                        </div>
                    </div>
                    
                    <div class="flex items-start space-x-3 sm:space-x-4">
                        <div class="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-purple-100 flex items-center justify-center">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-base sm:text-lg font-semibold text-gray-900 truncate">{{ $appointment->patient->name }}</p>
                            <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ $appointment->patient->phone }}</p>
                        </div>
                    </div>
                    
                    <div class="flex items-start space-x-3 sm:space-x-4">
                        <div class="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-green-100 flex items-center justify-center">
