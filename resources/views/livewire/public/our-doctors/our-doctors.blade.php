<div class="min-h-screen bg-gray-50" x-data="{ scrolled: false }" x-init="window.addEventListener('scroll', () => { scrolled = window.scrollY > 300 })">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
            <form wire:submit.prevent="loadDoctors">
                ...
                <div class="flex items-center mb-4">
                    ...
                    <button type="submit"
                        class="ml-3 bg-brand-blue-600 text-white px-5 py-3 rounded-lg hover:bg-brand-blue-700 transition-colors">
                        Search
                    </button>
                </div>
                ...
                <div class="hidden md:flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <!-- More Filters Button -->
                        <button type="button" @click="$dispatch('open-filter-drawer')"
                            class="flex items-center px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">
                ...
                <div class="md:hidden flex justify-between mt-4">
                    <button type="button" @click="$dispatch('open-filter-drawer')"
                        class="flex items-center px-4 py-2 bg-gray-100 rounded-lg text-sm text-gray-700">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                ...
                <div class="flex flex-wrap gap-2 mt-4">
                    @if($departmentSlug)
                        <span class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-medium bg-brand-blue-100 text-brand-blue-800">
                            ...
                            <button type="button" wire:click="$set('department_id', null)" class="ml-1.5 text-brand-blue-600 hover:text-brand-blue-800">
                    ...
                    <button type="button" wire:click="resetFilters" 
                            class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200">
                        Clear all
                    </button>
                </div>
                @endif
        ...
        <div x-show="scrolled" 
            ...
            <div class="max-w-7xl mx-auto flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <!-- Department Dropdown (Simplified) -->
                    <select wire:model.live="department_id"
                        class="text-sm border border-gray-300 rounded-md bg-white py-1.5 px-3">
                        <option value="">All Specialties</option>
                        @foreach ($departments as $department)
                            <option value="{{ $department->slug }}">{{ $department->name }}</option>
                        @endforeach
                    ...
                    <!-- Quick Filters -->
                    <button type="button" @click="$dispatch('open-filter-drawer')"
                        class="flex items-center px-3 py-1.5 border border-gray-300 rounded-md text-xs sm:text-sm text-gray-700 hover:bg-gray-50">
                ...
                <div class="flex items-center">
                    <select wire:model.live="sortBy"
                        class="text-xs sm:text-sm border border-gray-300 rounded-md bg-white py-1.5 px-3">
                        <option value="recommended">Sort: Recommended</option>
                        <option value="rating">Sort: Rating</option>
                        <option value="fee_low">Sort: Low Price</option>
                        <option value="fee_high">Sort: High Price</option>
            ...
            <div class="flex-1">
                ...
                <div class="space-y-4">
                    @forelse ($doctors as $doctor)
                        <div class="bg-white rounded-lg border border-gray-200 hover:border-brand-blue-300 transition-all shadow-sm">
                            <div class="p-4">
    ...
    <div x-data="{ open: false }" 
         ...
         class="fixed inset-y-0 right-0 z-40 w-full sm:w-96 bg-white shadow-xl overflow-y-auto"
         style="display: none;">
        
        <!-- Drawer Header -->
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            <h3 class="text-lg font-bold text-gray-900">Filter Options</h3>
            <button @click="open = false" class="p-2 rounded-full hover:bg-gray-100 text-gray-500">
        ...
        <div class="p-4 space-y-6">
            ...
            <div>
                ...
                <select wire:model.live="department_id" class="w-full p-2 border border-gray-300 rounded-lg">
                    <option value="">All Specialties</option>
                    @foreach ($departments as $department)
                        <option value="{{ $department->slug }}">{{ $department->name }}</option>
                    @endforeach
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        <button type="button" wire:click="$set('minRating', {{ $i }})"
                                class="w-8 h-8 rounded-full flex items-center justify-center 
                                      {{ $minRating >= $i ? 'bg-brand-blue-600 text-white' : 'bg-gray-100 text-gray-400' }}">
                            {{ $i }}
                        </button>
                    @endfor
        ...
        <div class="p-4 border-t border-gray-200">
            <div class="flex gap-3">
                ...
                <button wire:click="loadDoctors" @click="open = false"
    ...
    <!-- Modal backdrop -->
    <div x-data x-show="$store.filterDrawer?.open" 
         x-on:open-filter-drawer.window="$store.filterDrawer.open = true"
         x-transition:enter="transition ease-out duration-300"
         ...
         @click="$store.filterDrawer.open = false"
         class="fixed inset-0 bg-gray-900/30 backdrop-blur-sm z-30"
         style="display: none;">
    </div>
