<div class="min-h-screen bg-gradient-to-br from-teal-50 to-cyan-50 px-4 py-8 sm:py-12">
    <div class="container mx-auto max-w-7xl">
        ...
        <div class="text-center mb-12">
            ...
            <p class="text-teal-600 max-w-2xl mx-auto text-lg">Connect with trusted healthcare professionals in your area</p>
        </div>
 ...
 Section -->
            <aside class="w-full lg:w-1/4 bg-white p-6 rounded-xl shadow-sm border border-teal-100 lg:sticky top-24 h-fit">
            ...
            <section class="w-full lg:w-3/4">
                ...
                <div class="space-y-6">
                    @forelse ($doctors as $doctor)
                        <div class="bg-white p-6 rounded-xl shadow-sm border border-teal-100 hover:shadow-md transition-all duration-300 group">
                            <div class="flex flex-col md:flex-row gap-6">
                                <!-- Doctor Image -->
                                <div class="flex-shrink-0 relative">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($doctor->user->name) }}&background=0D9488&color=fff&size=128"
                                        alt="{{ $doctor->user->name ?? 'Doctor' }}"
                                        class="w-24 h-24 rounded-full border-4 border-teal-100 object-cover shadow">
                                    <div class="absolute -bottom-2 right-0 bg-orange-500 text-white rounded-full p-1 shadow-md">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="flex-grow">
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                                        <h3 class="text-xl font-bold text-teal-800">{{ $doctor->user->name ?? 'N/A' }}</h3>
                                        <span class="bg-teal-100 text-teal-700 text-sm px-3 py-1 rounded-full font-medium">{{ $doctor->department->name ?? 'N/A' }}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 mb-3">
                                        <div class="flex text-orange-400">
                                            ...
                                            <span class="ml-1 text-teal-700 font-medium">{{ $doctor->rating ?? '4.9' }}</span>
                                            <span class="text-teal-400 ml-1">({{ $doctor->review_count ?? '24' }} reviews)</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex gap-2">
                                        <span class="bg-cyan-100 text-cyan-800 text-xs px-2 py-1 rounded">General Checkup</span>
                                        <span class="bg-amber-100 text-amber-800 text-xs px-2 py-1 rounded">Pediatric</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex-shrink-0 flex flex-col items-end gap-4">
                                    ...
                                    <div class="flex flex-col gap-2 w-full sm:w-auto">
                                        ...
                                        <a wire:navigate href="{{ route('doctor-detail', ['doctor_id' => $doctor->id]) }}"
