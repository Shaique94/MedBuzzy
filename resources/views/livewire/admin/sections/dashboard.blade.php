	...
	<main class="flex-1 overflow-x-hidden overflow-y-auto min-h-screen">
		<div class="container mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 max-w-7xl">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4 sm:mb-6">
				...
				<div class="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
					<div class="flex items-center justify-between mb-2">
						...
						<div class="text-right">
							<div class="text-xl sm:text-2xl font-semibold text-gray-800">{{ $appointmentsCount ?? 2 }}</div>
				...
				<div class="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
					<div class="flex items-center justify-between mb-2">
						<div class="bg-green-600 p-2 rounded-lg text-white">
							<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857"></path>
							</svg>
						</div>
						<div class="text-right">
				...
				<div class="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
					<div class="flex items-center justify-between mb-2">
						...
						<div class="text-right">
							<div class="text-xl sm:text-2xl font-semibold text-gray-800">₹{{ number_format($totalRevenue ?? 50000, 0) }}</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
				...
				<div class="bg-white rounded-lg p-3 sm:p-4 border border-gray-100">
					<div class="flex items-center justify-between mb-2">
						...
						<div class="flex space-x-1">
							<button class="px-2 py-1 text-xs font-medium bg-purple-50 text-purple-700 rounded">6M</button>
							<button class="px-2 py-1 text-xs font-medium text-gray-600 rounded hover:bg-gray-50">1Y</button>
						</div>
					</div>
					<div class="relative h-40 sm:h-48 lg:h-64">
			...
			<div class="bg-white rounded-lg border border-gray-100 overflow-hidden">
				<div class="px-4 py-3 border-b border-gray-200 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
					<div>
					...
					<div class="flex items-center gap-2">
						...
						<a wire:navigate href="{{ route('admin.appointment') }}" class="inline-flex items-center px-2 py-1 border border-blue-200 rounded text-xs font-medium text-blue-700 bg-blue-50">View All</a>
					</div>
				</div>
					...
					@forelse ($appointments as $appointment)
						<li class="p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
							<div class="flex items-start sm:items-center gap-3">
								<div class="h-10 w-10 rounded-md bg-blue-600 flex items-center justify-center text-white font-semibold text-sm">
									{{ substr($appointment->patient->name, 0, 1) }}
								</div>
								<div>
									<p class="text-sm font-medium text-gray-900">{{ $appointment->patient->name }}</p>
									<p class="text-xs text-gray-500">Dr. {{ $appointment->doctor->user->name }} • {{ $appointment->doctor->department->name ?? '' }}</p>
								</div>
							</div>
							...
							<div class="flex items-center gap-4 text-xs text-gray-600">
								...
								<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $appointment->status === 'confirmed' ? 'bg-green-50 text-green-800' : ($appointment->status === 'pending' ? 'bg-yellow-50 text-yellow-800' : 'bg-gray-50 text-gray-700') }}">
									{{ ucfirst($appointment->status ?? 'pending') }}
								</span>
							</div>

							<div class="flex items-center gap-2">
								<a wire:navigate href="{{ route('appointment.receipt', ['appointment' => $appointment->id]) }}" target="_blank" class="px-3 py-1 text-xs text-blue-600 border border-transparent rounded hover:bg-blue-50">Details</a>
								<a wire:navigate href="{{ route('admin.update.appointment', $appointment->id) }}" class="px-3 py-1 text-xs text-amber-600 border border-transparent rounded hover:bg-amber-50">Edit</a>
							</div>
						</li>
					@empty
						<li class="p-6 text-center">
							<p class="text-sm text-gray-500">No appointments scheduled</p>
							<a wire:navigate href="{{ route('admin.add.appointment') }}" class="inline-flex items-center px-3 py-1.5 mt-3 border border-transparent rounded text-sm font-medium text-white bg-blue-600">Schedule Appointment</a>
						</li>
					@endforelse
				</ul>
			</div>
