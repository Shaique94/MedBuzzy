<div class="bg-white shadow-md px-6 py-4 flex items-center justify-between">
    <!-- Left: Page title and mobile menu button -->
    <div class="flex items-center">
        <!-- Mobile menu button -->
        <div class="md:hidden">
            <button onclick="toggleSidebar()" class="text-gray-700 focus:outline-none"  aria-label="Open navigation menu">
    ...
    <div class="flex items-center space-x-4">
        ...
        <button class="text-gray-500 hover:text-gray-700 focus:outline-none relative" aria-label="Notifications (3 unread)">
            ...
            <div class="flex items-center cursor-pointer space-x-2 group" @click="open = !open">
                @if($doctorImage)
                    <img class="h-10 w-10 rounded-full object-cover border-2 border-indigo-100 shadow-md group-hover:border-indigo-200 transition-all duration-200"
                         src="{{ $doctorImage }}?v={{ time() }}"
                         alt="Doctor Profile Image"
                         wire:key="header-profile-image-{{ $userId }}-{{ now()->timestamp }}">
                @else
